...
$pname = $_POST['packagename'];
    ...
    // Kiểm tra sự tồn tại của gói tour với tên tương tự
    $checkDuplicateQuery = "SELECT * FROM TblTourPackages WHERE PackageName = :pname";
    $checkDuplicateStmt = $dbh->prepare($checkDuplicateQuery);
    $checkDuplicateStmt->bindParam(':pname', $pname, PDO::PARAM_STR);
    $checkDuplicateStmt->execute();

if ($checkDuplicateStmt->rowCount() > 0) {
    // Gói tour đã tồn tại, hiển thị thông báo lỗi
    $error = "Gói tour với tên đã tồn tại. Vui lòng chọn tên khác.";
} else {
                                    ...
                                    <div class="row">
                                        <div class="form-group col-md-6">
                                            ...
                                            <div class="col-sm-12 pl-0 pr-0">
                                                <input type="text" class="form-control" name="packagename" id="packagename" placeholder="Tên tour du lịch" required>
                                            </div>
                                        </div>
