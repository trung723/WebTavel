...
?>
<div class="card-body">
    <h4 class="card-title">Cập nhật biểu mẫu người dùng </h4>
    ...
    if($query->rowCount() > 0)
    {
      ...
      { 
        ...
        <div class="form-group">
            <label for="exampleInputName1">Tên </label>
            <input type="text" name="phone2" class="form-control" id="phone2" value="<?php  echo $row->MobileNumber;?>"
                required>
        </div>
        <div class="form-group">
            <label for="exampleInputName1">Email</label>
            <input type="text" name="email2" class="form-control" id="email2" value="<?php  echo $row->Email;?>"
                required>
        </div>
        <?php $cnt=$cnt+1;
      }
    } ?>
        <button type="submit" name="saveupdates" class="btn btn-primary btn-fw mr-2">Cập nhật</button>
